...
| `TEAMS_MCP_SELF_ID` | no | Last-resort operator seed for the signed-in account's own AAD id, used internally by `send_chat_file`'s self-exclusion (`resolveSelfId` — see `send_chat_file`'s row above). Normally unnecessary: the id is resolved once from `/me` and persisted next to the token cache with no TTL, so only the first process on a fresh install pays the live lookup. Must be GUID-shaped or it is ignored |
