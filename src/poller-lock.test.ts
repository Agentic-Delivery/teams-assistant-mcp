...
/**
 * The twin-daemon race carried over from PR #8: two servers on one shared account meant every
 * poll interval carried double the Graph traffic, and under throttling each daemon's retries fed
 * the other's penalty window. The lock makes "one poller per inbox" a checked invariant instead
 * of an assumption about how many processes happen to be running.
 */
